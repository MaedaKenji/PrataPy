...
def calculate_crown_area(tree_detections, image, gsd):
    ...
    for idx, detection in enumerate(tree_detections.iterrows(), 1):
        ...
        if contours:
            ...
            tree_label = f"#{idx}"
            cv2.putText(original_image, tree_label, (x1, y1-30), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 255), 2)
            
            # Add area information to the image
            text = f"{crown_area_square_meters:.2f} m2"
            # cv2.putText(original_image, text, (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 0), 2)
...
def main():
    ...
    tree_detections, image = detect_trees(model, args.image, args.conf_thres, args.iou_thres)
